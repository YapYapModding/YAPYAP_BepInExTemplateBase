...
Once installed, the template will be available as `YAPYAP BepInEx Plugin` with an alias `yapyapmod`.

## Creating a Project

Open a terminal in your YAPYAP modding directory, and run:
...
```sh
dotnet new yapyapmod --output ModName --guid com.github.YourAccount.ModName --ts-team YourThunderstoreTeam
```
...
>
> You can run `dotnet new yapyapmod --help` to see all available options.
...
```sh
~/Workspace/YAPYAP$ dotnet new yapyapmod --output MyCoolMod --guid com.github.darmuh.MyCoolMod --ts-team darmuh
The template "YAPYAP BepInEx Plugin" was created successfully.

~/Workspace/YAPYAP$ cd MyCoolMod/
~/Workspace/YAPYAP/MyCoolMod$ tree
.
├── CHANGELOG.md
